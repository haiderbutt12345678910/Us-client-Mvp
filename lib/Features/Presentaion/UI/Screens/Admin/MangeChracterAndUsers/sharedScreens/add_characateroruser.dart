...
class AddChratcterOrUser extends StatefulWidget {
  final String id;
  const AddChratcterOrUser({super.key, required this.id});

  @override
  // ignore: library_private_types_in_public_api
  _AddChratcterOrUserState createState() => _AddChratcterOrUserState();
}

class _AddChratcterOrUserState extends State<AddChratcterOrUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        body: BlocConsumer<AddCharaterOrUserCubit, BlocStates>(
    ...
    if (_formKey.currentState!.validate()) {
      if (widget.id == "u") {
        UserEntity userEntity = UserEntity(
            ...
            uid: const Uuid().v4(),
            userName: _name.text);
        BlocProvider.of<AddCharaterOrUserCubit>(context)
            .addCharacterOrUser(userEntity, null, null);
      } else {
        if (image == null) {
          var snackBar = const SnackBar(
            duration: Duration(seconds: 2),
          ...
          CharacterEntity characterEntity = CharacterEntity(
              imageUrl: "", uid: const Uuid().v4(), name: _name.text);
          BlocProvider.of<AddCharaterOrUserCubit>(context)
              .addCharacterOrUser(null, characterEntity, image);
        }
      }
    }
  }
